...
bool FastPlannerManager::kinodynamicReplan(const Eigen::Vector3d& start_pt,
  ...
  if (status == KinodynamicAstar::NO_PATH) {
    ROS_ERROR_THROTTLE( 1, "search 1 fail");
    ROS_ERROR_THROTTLE( 1, "start: %f, %f, %f", start_pt(0), start_pt(1), start_pt(2));
    ROS_ERROR_THROTTLE( 1, "end: %f, %f, %f", end_pt(0), end_pt(1), end_pt(2));
    // Retry
    kino_path_finder_->reset();
    status = kino_path_finder_->search(start_pt, start_vel, start_acc, end_pt, end_vel, false);
    if (status == KinodynamicAstar::NO_PATH) {
      ROS_INFO_THROTTLE( 1, "[Kino replan]: Can't find path.");
      return false;
    }
  }
