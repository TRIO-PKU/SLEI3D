...
void KinoReplanFSM::init(ros::NodeHandle& nh) {
  ...
  /* callback */
  exec_timer_ = nh.createTimer(ros::Duration(0.04), &KinoReplanFSM::execFSMCallback, this);
  ...
  waypoint_server_ = nh.advertiseService("/waypoint_generator/waypoints", &KinoReplanFSM::waypointCallback, this);
  odom_sub_ = nh.subscribe("/odom_world", 1, &KinoReplanFSM::odometryCallback, this);

  replan_pub_ = nh.advertise<std_msgs::Empty>("/planning/replan", 10);
  new_pub_ = nh.advertise<std_msgs::Empty>("/planning/new", 10);
  bspline_pub_ = nh.advertise<bspline::Bspline>("/planning/bspline", 10);
  finish_pub_ = nh.advertise<std_msgs::Empty>("/finish_fastplanning", 10);
}

bool KinoReplanFSM::waypointCallback(msgs::waypointRequest& req, msgs::waypointResponse& res) {
  // if (req.path.poses[0].pose.position.z < -0.1) return false;
  //打印ros_error drone_id + "fast_planner_node get trigger"
  ROS_ERROR("Drone %s fast_planner_node get trigger", ros::this_node::getNamespace().c_str());
  cout << "Triggered!" << endl;
  trigger_ = true;
  ...
  res.success = true;
  ROS_ERROR("Drone %s fast_planner_node return true", ros::this_node::getNamespace().c_str());
  return true;
}
...
void KinoReplanFSM::execFSMCallback(const ros::TimerEvent& e) {
  ...
  string drone_id = ros::this_node::getNamespace();
