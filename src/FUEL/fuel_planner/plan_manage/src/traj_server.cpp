...
#include "msgs/get_pos_on_bspline.h"
...
bool get_ideal_yaw = false;
float ideal_yaw = 0.0;
geometry_msgs::Point32 ideal_pos = geometry_msgs::Point32();
bool get_ideal_pos;
...
void visCallback(const ros::TimerEvent& e) {
  Eigen::Vector4d GCS_COLOR(0.0, 0.0, 1.0, 0.8);
  Eigen::Vector4d EXP_COLOR(0.0, 0.5, 1.0, 0.8);
  Eigen::Vector4d INS_COLOR(0.0, 1.0, 1.0, 0.8);

  Eigen::Vector4d INS_1_COLOR(1.0, 0.078, 0.576, 1.0);
  Eigen::Vector4d INS_2_COLOR(0.72, 0.33, 0.827, 1.0);
  Eigen::Vector4d INS_3_COLOR(1.0, 0.75, 0.75, 1.0);
  string ns = ros::this_node::getNamespace();
  Eigen::Vector4d TRAJ_COLOR(1.0, 0.0, 0.0, 0.8);
  if (ns == "/ins_1") {
    TRAJ_COLOR = INS_1_COLOR;
  } else if (ns == "/ins_2") {
    TRAJ_COLOR = INS_2_COLOR;
  } else if (ns == "/ins_3") {
    TRAJ_COLOR = INS_3_COLOR;
  } else if (ns == "/gcs") {
    TRAJ_COLOR = GCS_COLOR;
  } else if (ns == "/exp_1") {
    TRAJ_COLOR = EXP_COLOR;
  } else {
    TRAJ_COLOR = INS_COLOR;
  }


  // Draw the executed traj (desired state)
  displayTrajWithColor(traj_cmd_, 0.05, TRAJ_COLOR, pub_traj_id_);
}
...
bool curPosOnBsplineCallback(msgs::get_pos_on_bspline::Request& req, msgs::get_pos_on_bspline::Response& res) {
  double t = req.time.toSec();
  double t_cur = (req.time - start_time_).toSec();
  bspline::Bspline bspline = req.bspline;
  std::vector<fast_planner::NonUniformBspline> traj;

  // 读取 bspline
  Eigen::MatrixXd pos_pts(bspline.pos_pts.size(), 3);
  ...
  for (int i = 0; i < bspline.pos_pts.size(); ++i) {
    pos_pts(i, 0) = bspline.pos_pts[i].x;
    pos_pts(i, 1) = bspline.pos_pts[i].y;
    pos_pts(i, 2) = bspline.pos_pts[i].z;
  }
  NonUniformBspline pos_traj(pos_pts, bspline.order, 0.1);
  pos_traj.setKnot(knots);

  traj.push_back(pos_traj);
  double traj_duration = traj[0].getTimeSum();

  // 计算对应时间的位置
  Eigen::Vector3d pos;
  if (t_cur < traj_duration && t_cur >= 0.0) {
    // Current time within range of planned traj
    pos = traj[0].evaluateDeBoorT(t_cur);
    ...
    pos = traj[0].evaluateDeBoorT(0.0);
  } else {
    string id = ros::this_node::getNamespace();
    string log = id + ": [Traj server]: invalid time.";
    ROS_ERROR(log.c_str());
  }
  res.pos.x = pos(0);
  res.pos.y = pos(1);
...
void yawCallback(std_msgs::Float32 msg)
{
  ideal_yaw = msg.data;
  get_ideal_yaw = true;
}

void posCallback(geometry_msgs::Point32 msg)
{
  ideal_pos = msg;
  get_ideal_pos = true;
}
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber yaw_sub = node.subscribe("ideal_yaw", 10, yawCallback);
  ros::Subscriber pos_sub = node.subscribe("ideal_pos", 10, posCallback);
  ...
  ros::ServiceServer pos_on_bspline_srv = node.advertiseService("get_pos_on_bspline", curPosOnBsplineCallback);
