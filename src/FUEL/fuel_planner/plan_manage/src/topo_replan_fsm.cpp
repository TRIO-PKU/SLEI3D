...
void TopoReplanFSM::init(ros::NodeHandle& nh) {
  ...
  waypoint_server_ = nh.advertiseService("/waypoint_generator/waypoints", &TopoReplanFSM::waypointCallback, this);
...
bool TopoReplanFSM::waypointCallback(msgs::waypointRequest& req, msgs::waypointResponse& res) {
  ...
  if (target_type_ == TARGET_TYPE::REFENCE_PATH) {
		...
		waypoint_num_ = req.path.poses.size();
		
    for (int i = 0; i < waypoint_num_; ++i) {
      Eigen::Vector3d pt;
      pt(0) = req.path.poses[i].pose.position.x;
      pt(1) = req.path.poses[i].pose.position.y;
      pt(2) = req.path.poses[i].pose.position.z;
      global_wp.push_back(pt);
    }
  } else {
    if (target_type_ == TARGET_TYPE::MANUAL_TARGET) {
      target_point_(0) = req.path.poses[0].pose.position.x;
      target_point_(1) = req.path.poses[0].pose.position.y;
      target_point_(2) = req.path.poses[0].pose.position.z;
