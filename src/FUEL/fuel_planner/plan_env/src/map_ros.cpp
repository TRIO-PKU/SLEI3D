...
void MapROS::init() {
  ...
  volume_service = node_.advertiseService("volume", &MapROS::volumeCallback, this);
...
bool MapROS::volumeCallback(msgs::volume::Request& req, msgs::volume::Response& res){
  ...
  res.volume = static_cast<float>(known_volumn);
  res.surface_area = static_cast<float>(surface_area);
  return true;
}
...
void MapROS::publishMapAll() {
  ...
  // //将字符串drone_id的第2个到第4个字符提取出来
  // std::string drone_id = ros::this_node::getNamespace();
  // std::string drone_type = drone_id.substr(1, 3);
  // if (drone_type != "exp") {
  //   return;
  // }
  ...
  // // std::string file_name = "/home/xz405/ExpInsSim/src/FUEL/fuel_planner/exploration_manager/resource/" + drone_id + ".txt";
...
void MapROS::init_file() {
  std::string drone_id = ros::this_node::getNamespace();
  std::string drone_type = drone_id.substr(1, 3);
    if (drone_type != "exp") {
      ROS_WARN("%s", drone_id.c_str());
      ROS_ERROR("drone_id is not correct");
      return;
    }
  // TODO: change the file path
  //若有文件则清空文件内容，若无文件则创建文件
  std::string file_name = "/home/xz405/ExpInsSim/src/FUEL/fuel_planner/exploration_manager/resource/" + drone_id + ".txt";
  ofstream file(file_name,
                ios::trunc);
  if (!file) {
