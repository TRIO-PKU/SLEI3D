...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  nh.param("sdf_map/map_size_x", x_size, 2.0);
  nh.param("sdf_map/map_size_y", y_size, 2.0);
  ...
  nh.param("sdf_map/map_origin_x", mp_->map_origin_x, 0.0);
  nh.param("sdf_map/map_origin_y", mp_->map_origin_y, 0.0);
  nh.param("sdf_map/map_origin_z", mp_->map_origin_z, 0.0);

  mp_->local_bound_inflate_ = max(mp_->resolution_, mp_->local_bound_inflate_);
  mp_->resolution_inv_ = 1 / mp_->resolution_;
  // mp_->map_origin_ = Eigen::Vector3d(-x_size / 2.0, -y_size / 2.0, mp_->ground_height_);
  mp_->map_origin_ = Eigen::Vector3d(mp_->map_origin_x, mp_->map_origin_y, mp_->map_origin_z);
  ...
  for (int x = md_->local_bound_min_(0); x <= md_->local_bound_max_(0); ++x)
    for (int y = md_->local_bound_min_(1); y <= md_->local_bound_max_(1); ++y)
      for (int z = md_->local_bound_min_(2); z <= md_->local_bound_max_(2); ++z) {
        int index = toAddress(x, y, z);
        if (index >= 0 && index < md_->occupancy_buffer_inflate_.size()) {
  ...
  for (int x = md_->local_bound_min_(0); x <= md_->local_bound_max_(0); ++x)
    for (int y = md_->local_bound_min_(1); y <= md_->local_bound_max_(1); ++y)
      for (int z = md_->local_bound_min_(2); z <= md_->local_bound_max_(2); ++z) {
        ...
        if (id1 >= 0 && id1 < md_->occupancy_buffer_.size() && md_->occupancy_buffer_[id1] > mp_->min_occupancy_log_) {
          inflatePoint(Eigen::Vector3i(x, y, z), inf_step, inf_pts);

          for (auto inf_pt : inf_pts) {
            int idx_inf = toAddress(inf_pt);
            if (idx_inf >= 0 && idx_inf < md_->occupancy_buffer_inflate_.size() &&
                idx_inf <
