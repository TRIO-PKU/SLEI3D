...
void FastExplorationManager::initialize(ros::NodeHandle& nh) {
  ...
  nh.param("drone_id", ep_->drone_id, string("1"));
  ...
  ofstream par_file(ep_->tsp_dir_ + "/single" + ep_->drone_id + ".par");
  par_file << "PROBLEM_FILE = " << ep_->tsp_dir_ << "/single" << ep_->drone_id << ".tsp\n";
  par_file << "GAIN23 = NO\n";
  par_file << "OUTPUT_TOUR_FILE =" << ep_->tsp_dir_ << "/single" << ep_->drone_id << ".txt\n";
  par_file << "RUNS = 1\n";
...
int FastExplorationManager::planExploreMotion(
  ...
  if (ed_->in_meeting) {
    geometry_msgs::PoseStamped meeting_pose = ed_->meeting_pose;
    next_pos[0] = meeting_pose.pose.position.x;
    next_pos[1] = meeting_pose.pose.position.y;
    next_pos[2] = meeting_pose.pose.position.z;
    next_yaw = atan2(2 * (meeting_pose.pose.orientation.w * meeting_pose.pose.orientation.z +
                          meeting_pose.pose.orientation.x * meeting_pose.pose.orientation.y),
                     1 - 2 * (meeting_pose.pose.orientation.y * meeting_pose.pose.orientation.y +
                              meeting_pose.pose.orientation.z * meeting_pose.pose.orientation.z));
  }

  // TODO； 向 meeting_manager 汇报算出的 next_pos 和 next_yaw, 判断是否继续 exploration
  ...
  int res = planTraj(pos, vel, acc, yaw, next_pos, next_yaw);
...
int FastExplorationManager::planTraj(const Vector3d& pos, const Vector3d& vel, const Vector3d& acc, const Vector3d& yaw, const Vector3d& next_pos, const double& next_yaw)
{
  ...
  if (planner_manager_->path_finder_->search(pos, next_pos) != Astar::REACH_END) {
    ...
    ROS_ERROR("Start: %lf, %lf, %lf", pos[0], pos[1], pos[2]);
    ROS_ERROR("End: %lf, %lf, %lf", next_pos[0], next_pos[1], next_pos[2]);
    return FAIL;
  }
  ed_->path_next_goal_ = planner_manager_->path_finder_->getPath();
  shortenPath(ed_->path_next_goal_);

  const double radius_far = 100;
  ...
  if (planner_manager_->local_data_.position_traj_.getTimeSum() < time_lb - 0.1)
    ROS_INFO("Lower bound not satified!");
...
void FastExplorationManager::findGlobalTour(
  ...
  for (int n = 1; n < dimension; ++n) {
    for (int i = 1; i < dimension; ++i) {
      ...
      double min_cost = 100000;
      if (cost_mat(last_idx, i) < min_cost) {
        min_idx = i;
        min_cost = cost_mat(0, i);
      }
    } 
    last_idx = min_idx;
    int id = min_idx - 1;
    gone_idxs.push_back(id);
  ...
  ofstream prob_file(ep_->tsp_dir_ + "/single" + ep_->drone_id + ".tsp");
  // Problem specification part, follow the format of TSPLIB

  string prob_spec = "NAME : single" + ep_->drone_id + "\nTYPE : ATSP\nDIMENSION : " + to_string(dimension) +
      "\nEDGE_WEIGHT_TYPE : "
  ...
  // Call LKH TSP solver
  solveTSPLKH((ep_->tsp_dir_ + "/single" + ep_->drone_id + ".par").c_str());

  // Read optimal tour from the tour section of result file
  ifstream res_file(ep_->tsp_dir_ + "/single" + ep_->drone_id + ".txt");
  string res;
