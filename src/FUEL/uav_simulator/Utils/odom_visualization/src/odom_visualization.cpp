...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  if (robot_type=="gcs"){
    mesh_resource = "package://odom_visualization/meshes/little_car.stl";
  }
  else if (robot_type=="quadrotor"){
...
void cmd_callback(const quadrotor_msgs::PositionCommand cmd) {
  ...
  meshROS.color.b = color_b;
  if (robot_type=="gcs"){
    mesh_resource = "package://odom_visualization/meshes/gcs.stl";
  }
  else if (robot_type=="quadrotor"){
    mesh_resource = "package://odom_visualization/meshes/hummingbird.mesh";
  }
...
int main(int argc, char** argv) {
  ...
  //从参数服务器中获取参数"drone_type"的值，如果没有则使用默认值"quadrotor"
  n.param("robot_type", robot_type, std::string("gcs"));
